...
describe('PostService Real DB Test', () => {
  ...
  describe('createPost', () => {
    it('게시글 생성 후 회원 id로 조회하여 검증', async () => {
      ...
      //then
      expect(
        (await service.getPostsByWriterId(userId, BigInt(0))).postSummaries[0]
          .title,
      ).toEqual(title);
    });
  });
  ...
  describe('updateContent', () => {
    it('content를 변경한 후, content와 post_state를 검증한다.', async () => {
      ...
      const postId = (await service.getPostsByWriterId(userId, BigInt(0)))
        .postSummaries[0].id;
      ...
      expect(foundPost.post_state).toEqual($Enums.post_state.EDITED);
    });
  ...
  describe('searchPostsByTitle', () => {
    it('title의 첫글자로 검색후 검증', async () => {
      ...
      //when
      const keyword = 'te';
      const postPages = (await service.searchPostsByTitle(keyword, BigInt(0)))
        .postSummaries;
